...
interface ISuccessToastProps extends HTMLAttributes<HTMLDivElement> {
    heading?: ReactNode;
    description?: ReactNode;
    action?: IAction;
}

const SuccessToast = ({ className, heading, description, action }: ISuccessToastProps) => {
    return (
        <div className={cn("size-full flex items-center justify-between gap-x-4", className)}>
            <div className="flex flex-col gap-y-1">
                <h6 className="text-base font-semibold text-success">{heading ? heading : "Success"}</h6>
